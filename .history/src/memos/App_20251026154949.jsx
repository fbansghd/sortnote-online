...
function App() {
  ...
  const handleDeleteCategory = (realIndex) => {
    deleteCategory(realIndex);
    saveMemosToServer();
  };

  const handleAddTask = (catIdx, value) => {
    addTaskToCategory(catIdx, value);
    saveMemosToServer();
  };

  const handleDeleteTask = (categoryId, taskId) => {
    deleteTaskById(categoryId, taskId);
    saveMemosToServer();
  };

  const handleToggleTaskDone = (catIdx, taskId) => {
    toogleTaskDone(catIdx, taskId);
    saveMemosToServer();
  };
                        ...
                        <SortableCategory
                          ...
                          onDelete={() => {
                            if (window.confirm("本当にこのカテゴリを削除しますか？")) {
                              handleDeleteCategory(realIndex);
                            }
                          }}
                                ...
                                {(categoryItem.tasks || [])
                                  ...
                                  .map((taskItem) => (
                                    <SortableTask
                                      ...
                                      onToggle={() => handleToggleTaskDone(realIndex, taskItem.id)}
                                      onDelete={() => handleDeleteTask(categoryItem.id, taskItem.id)}
                                    />
                                  ))}
                              </SortableContext>
                                      ...
                                      <input
                                        ...
                                        onKeyDown={e => {
                                          if (e.key === "Enter") {
                                            e.preventDefault();
                                            e.stopPropagation();
                                            handleAddTask(realIndex, taskInputs[realIndex]); // 正しいインデックス
                                          }
                                        }}
                                      />
                                      <button
                                        className={styles.addBtn}
                                        onClick={() => handleAddTask(realIndex, taskInputs[realIndex])} // 正しいインデックス
                                      >
                                        add
                                      </button>
                                    </div>
